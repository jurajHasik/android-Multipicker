...
public class ImageBaseLoader extends AsyncTaskLoader<List<MediaEntityWrapper>> {
    ...
    static final String TAG = ImageBaseLoader.class.getSimpleName();
    ...
    public List<MediaEntityWrapper> loadInBackground() {
        ...
        try {
            ...
            Log.d(TAG, "ImageCursor Returned " + imageCursor.getCount() + " image files");

            final Uri baseThumbUri = MediaStore.Images.Thumbnails.getContentUri("external");
            Log.d(TAG, "ImageThumbnailsUri: " + baseThumbUri.toString());
            ...
            final String thumbSelection = MediaStore.Images.Thumbnails.KIND+" IN (?, ?, ?)";
            final String[] thumbSelectionArgs = {
                    String.valueOf(MediaStore.Images.Thumbnails.MICRO_KIND),
                    String.valueOf(MediaStore.Images.Thumbnails.MINI_KIND),
                    String.valueOf(MediaStore.Images.Thumbnails.FULL_SCREEN_KIND)};
            ...
            if ((imageCursor.getCount() > 0) && (thumbCursor.getCount() > 0)) {
                ...
                for (CursorJoiner.Result joinerResult : joiner) {
                    switch (joinerResult) {
                        case LEFT: {
                            ...
                            tmpEntity.masterId = imageCursor.getString(imageCursor.getColumnIndex(MediaStore.Files.FileColumns._ID));
                            tmpEntity.parent = imageCursor.getString(imageCursor.getColumnIndex(MediaStore.Files.FileColumns.PARENT));
                            tmpEntity.mimeType = imageCursor.getString(imageCursor.getColumnIndex(MediaStore.Files.FileColumns.MIME_TYPE));
                            tmpEntity.masterDataPath = imageCursor.getString(imageCursor.getColumnIndex(MediaStore.Files.FileColumns.DATA));
                            tmpEntity.size = imageCursor.getLong(imageCursor.getColumnIndex(MediaStore.Files.FileColumns.SIZE));
                        ...
                        case BOTH: {
                            ...
                            if(tmpEntity.thumbURI_type.equals(MultiPicker.ThumbURI_Type.NONE))
                                Log.d(TAG, "Thumbnail entry invalid -  image file does not exist, " +
                                        "falling back to original Image file");
                            else
                                Log.d(TAG, "FOUND Image Thumb for Image " + tmpEntity.masterId +
                ...
                Log.d(TAG, "Created Cursor with " + entityList.size() + " images+thumb");
            } else if(imageCursor.getCount() > 0) {
                Log.d(TAG, "No thumbnails returned - using only original Images");
    ...
    protected void onStartLoading() {
        ...
        if (takeContentChanged() || mData == null) {
            ...
            Log.d(TAG, "ImageBaseLoader.forceLoad called");
            forceLoad();
        }
    }
